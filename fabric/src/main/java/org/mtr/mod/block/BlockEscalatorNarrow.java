...
public abstract class BlockEscalatorNarrow extends BlockExtension implements IBlock, DirectionHelper {
    ...
    protected BlockEscalatorNarrow() {
...
//    }
    ...
    protected final EnumEscalatorNarrowOrientation getOrientation(BlockView world, BlockPos pos, BlockState state) {
        ...
        if ((isAhead || isAheadUp) && isBehind) {
            if (isAheadUp) return EnumEscalatorNarrowOrientation.TRANSITION_BOTTOM_1;
            if (isBehindDown2) return EnumEscalatorNarrowOrientation.TRANSITION_TOP;
            return EnumEscalatorNarrowOrientation.FLAT;
        } else if (isBehindDown1) {
            if (isBehindDown2) return EnumEscalatorNarrowOrientation.TRANSITION_BOTTOM_2;

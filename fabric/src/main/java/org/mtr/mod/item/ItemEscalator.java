...
import org.mtr.mod.block.BlockEscalatorLegacy;
import org.mtr.mod.block.BlockEscalatorLegacySide;
import org.mtr.mod.block.BlockEscalatorLegacyStep;
...
public class ItemEscalator extends ItemExtension implements IBlock {
	...
	public ActionResult useOnBlock2(ItemUsageContext context) {
		...
		if (frontState.getBlock().data instanceof BlockEscalatorLegacy) {
			if (IBlock.getStatePropertySafe(frontState, BlockEscalatorLegacy.FACING) == playerFacing.getOpposite()) {
		...
		final BlockState stepState = Blocks.ESCALATOR_STEP.get().getDefaultState().with(new Property<>(BlockEscalatorLegacyStep.FACING.data), playerFacing.data);
		...
		final BlockState sideState = Blocks.ESCALATOR_SIDE.get().getDefaultState().with(new Property<>(BlockEscalatorLegacySide.FACING.data), playerFacing.data);
